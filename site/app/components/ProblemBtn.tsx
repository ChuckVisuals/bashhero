...
export default function ProblemBtn({ id, title, description, level }: { id: string, title: string, description: string, level: Difficulty }) {
    ...
    return (
        <Link href={`/problems/${id}`}>
            <div className={`group relative border rounded-4xl h-64 w-86 bg-neutral-900 p-6 mx-4 hover:h-96 hover:w-129 hover:bg-neutral-800 hover:cursor-pointer duration-400 overflow-hidden ${borderColor[level]}`}>
                <h2 className="text-xl font-bold text-white">{title}</h2>
                <div className="prose prose-stone prose max-w-5xl prose-invert p-8 mt-4 border border-neutral-600 rounded-lg bg-neutral-800 max-h-9/10 overflow-hideen opacity-10 hover:opacity-100 duration-300">
